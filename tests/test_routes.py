...
class TestProductRoutes(TestCase):
    ...
    def test_update_product_with_non_existent_id(self):
        """It should return not found error when update a Product with non existing id"""
        # create a product to update
        test_product = ProductFactory()
        # send a self.client.post() request to the BASE_URL with a json payload of test_product.serialize()
        response = self.client.post(BASE_URL, json=test_product.serialize())
        ...
        new_product["description"] = "testing"
        # send a self.client.put() request to the BASE_URL with a json payload of new_product
        response = self.client.put(f"{BASE_URL}/{24000}", json=new_product)
        # assert that the resp.status_code is status.HTTP_200_OK
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_query_by_name(self):
        ...
        self.assertEqual(len(found), name_count)
    ...
    def test_query_by_availability(self):
        ...
        logging.debug("Available Products [%d] %s", count_available_products, available_products)
