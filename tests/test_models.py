...
class TestProductModel(unittest.TestCase):
    ...
    def test_read_a_product(self):
        ...
        # Creating a product with factory
        product = ProductFactory()
        product.id = None
        product.create()
        ...
        # Checking the properties of product
    ...
    def test_update_a_product(self):
        ...
        # Checking if has only one product and retrieving correct data
        products = Product.all()
        self.assertEqual(len(products), 1)
    ...
    def test_update_a_product_without_id(self):
        """It should raises an excepting when try to update a product without id"""
        # Creating a product with factory
        product = ProductFactory()
        product.id = None
        product.create()
    ...
    def test_find_by_price(self):
        ...
        # Call the find_by_price() method on the Product class to retrieve products from the database that have the specified category.
        found = Product.find_by_price(price)
        # Assert if the count of the found products matches the expected count.
        self.assertEqual(found.count(), count)
        # Use a for loop to iterate over the found products and assert that each product's price matches the expected price, to ensure that all the retrieved products have the correct category.
        for product in found:
            self.assertEqual(product.price, price)
    
    def test_find_by_price_when_price_is_a_string(self):
        """It should Find Products by Price even if price is a string"""
        products = ProductFactory.create_batch(10)
        for product in products:
            product.create()
        price = products[0].price
